...
import org.chiches.feign.ProductServiceClient;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderServiceImpl(OrderRepository orderRepository,
                            ModelMapper modelMapper, InterServiceRepository interServiceRepository, ProductServiceClient productServiceClient
                            //,
...
//        this.notificationService = notificationService;
    ...
    public ResponseEntity<OrderDTO> createOrder(OrderDTO orderDTO, Principal principal) {
        ...
        List<Long> ids = orderDTO.getOrderContentDTOs().stream()
                .map(OrderContentDTO::getDishItemId)
                .toList();

        List<DishItemDTO> prices = productServiceClient.getPricesByIds(ids).getBody();
        Map<Long, DishItemDTO> priceMap = prices.stream()
                .collect(Collectors.toMap(DishItemDTO::getId, Function.identity()));

        for (OrderContentDTO oc : orderDTO.getOrderContentDTOs()) {

            orderEntity.addOrderContent(priceMap.get(oc.getDishItemId()).getId(),
                    priceMap.get(oc.getDishItemId()).getPrice(),
                    oc.getQuantity());
        }
        try {
